...
namespace AutoLead
{
    public class Form1 : Form
	{
		...
		public string documentfolder = string.Concat(Environment.GetFolderPath(Environment.SpecialFolder.Personal), "\\");
                    ...
                    try
                    {
                        ...
                        while (enumerator.MoveNext())
                        {
                            ...
                            if (current.offerEnable)
                            {
                                ...
                                {
                                    if (!Uri.TryCreate(strArrays2[(new Random()).Next(0, strArrays2.Count<string>())], UriKind.Absolute, out invokeQuan.uri))
                                    {
                                        invokeQuan.flag1 = false;
                                    }
                                    else
                                    {
                                        invokeQuan.flag1 = (invokeQuan.uri.Scheme == Uri.UriSchemeHttp ? true : invokeQuan.uri.Scheme == Uri.UriSchemeHttps);
                                    }
                                    if (!invokeQuan.flag1)
                                    {
                                        this.cmd.setReferer("");
                                    ...
                                    {
                                        this.cmd.setReferer(invokeQuan.uri.GetComponents(UriComponents.SchemeAndServer, UriFormat.UriEscaped));
                                    }
        ...
        private invoke vip72Method(invoke invokeQuan)
        {
            ...
            while (true)
            {
                if (!(dynamic)(!invokeQuan.vip72Chung.vip72login(this.vipacc.username, this.vipacc.password, (int)this.numericUpDown1.Value)))
                {
                    break;
                }
                num2++;
            ...
            while (true)
            {
                ...
                if (!invokeQuan.vip72Chung.getip(this.listcountrycode.FirstOrDefault<countrycode>((countrycode x) => x.country == text3).code))
                {
                    break;
                }
                this.label1.Invoke(new MethodInvoker(() => {
                ...
                else if (invokeQuan.text == "Vip72")
                {
                    ...
                    if (this.vipacc != null)
                    {
                        ...
                        while (true)
                        {
                            ...
                            if (!invokeQuan.vip72Chung.getip(this.listcountrycode.FirstOrDefault<countrycode>((countrycode x) => x.country == text6).code))
                            {
                                break;
                            }
                            this.label1.Invoke(new MethodInvoker(() => {
						...
						Label3:
							...
							if (text == "SSH")
							{
								...
								while (true)
								{
									...
									ssh red = this.listssh.FirstOrDefault<ssh>((ssh x) => (!(x.live != "dead") || x.used ? false : x.country == text1));
									@checked = false;
									...
									while (true)
									{
										if ((this.listssh.ElementAt<ssh>(aqua).live == "dead" || this.listssh.ElementAt<ssh>(aqua).used ? false : this.listssh.ElementAt<ssh>(aqua).country == text1))
										{
											break;
										}
									...
									this.curip = red.IP;
									if (sshcommand.SetSSH(red.IP, red.username, red.password, this.ipAddressControl1.Text, this.numericUpDown1.Value.ToString(), ref this.bitproc))
									{
										break;
									}
									red.live = "dead";
							...
							else
							{
								...
								if (yellow != null)
								{
									...
									while (true)
									{
										if (!(dynamic)(!vip72Chung.vip72login(yellow.username, yellow.password, (int)this.numericUpDown1.Value)))
										{
											break;
										}
										num2++;
									...
									while (true)
									{
										...
										if (!vip72Chung.getip(this.listcountrycode.FirstOrDefault<countrycode>((countrycode x) => x.country == text4).code))
										{
											break;
										}
										this.label1.Invoke(new MethodInvoker(() => {
					...
					Label2:
						...
						try
						{
							while (enumerator3.MoveNext())
							{
								...
								{
									Thread.Sleep(1000);
									this.label1.Invoke(new MethodInvoker(() => this.label1.Text = string.Concat("Application will be closed in ", (value1 - j - 1).ToString(), " seconds")));
								}
								...
								this.label1.Invoke(new MethodInvoker(() => {
									if ((!this.checkBox6.Checked ? false : this.comboBox3.SelectedIndex != -1))
									{
										...
										{
											script = this.listscript.ElementAt<AutoLead.Script>(this.comboBox3.SelectedIndex);
										}
										else
										{
											List<AutoLead.Script> list = (
												from x in this.listscript
												where (x.slot == this.comboBox3.SelectedIndex ? true : this.comboBox3.SelectedIndex == 0)
												select x).ToList<AutoLead.Script>();
											script = list.ElementAt<AutoLead.Script>((new Random()).Next(0, list.Count));
										}
		...
		Label5:
            ...
            //goto Label3;
            MessageBox.Show("No Lable3 to goto line 2974!", Application.ProductName, MessageBoxButtons.OK, MessageBoxIcon.Hand);
        ...
        Label6:
            ...
            //goto Label3;
            MessageBox.Show("No Lable3 to goto line 2987!", Application.ProductName, MessageBoxButtons.OK, MessageBoxIcon.Hand);
        }
		...
		{
			string str = this._listcountry.FirstOrDefault<string>((string x) => x == this.comboBox2.Text) ?? this._listcountry.FirstOrDefault<string>((string x) => x.Contains(this.comboBox2.Text));
			this.carrierBox.Text = str;
			this.comboBox1.Text = this.listlanguagecode.FirstOrDefault<languagecode>((languagecode x) => x.langcode == this.listcountrycodeiOS.FirstOrDefault<countrycodeiOS>((countrycodeiOS y) => y.countryname == this.comboBox2.Text).languageCode).langname;
		}
			...
			for (int i = 0; i < (int)strArrays.Length; i++)
			{
				...
				if (strArrays1.Count<string>() >= 3)
				{
					...
					if (flag)
					{
						...
						ListViewItem listViewItem = new ListViewItem(new string[] { _ssh.IP, _ssh.username, _ssh.password, _ssh.country });
						this.listView2.Items.Add(listViewItem);
					}
				}
			}
		...
		private void loadssh()
		{
			if (this.DeviceInfo.SerialNumber != null)
			{
				this.listssh.Clear();
				this.listView2.Items.Clear();
				if (File.Exists(string.Concat(AppDomain.CurrentDomain.BaseDirectory, this.DeviceInfo.SerialNumber, "\\ssh.dat")))
				{
					string[] strArrays = File.ReadAllText(string.Concat(AppDomain.CurrentDomain.BaseDirectory, this.DeviceInfo.SerialNumber, "\\ssh.dat")).Split(new string[] { "\r\n" }, StringSplitOptions.None);
					for (int i = 0; i < (int)strArrays.Length; i++)
					{
						string str = strArrays[i];
						string[] strArrays1 = str.Split(new string[] { "||" }, StringSplitOptions.None);
			...
			Label2:
                ...
                if (text != "SSH")
				{
					...
					if (yellow != null)
					{
						...
						while (true)
						{
							string str = "";
							this.label1.Invoke(new MethodInvoker(() => str = this.comboBox5.Text));
							if (!vip72Chung.getip(this.listcountrycode.FirstOrDefault<countrycode>((countrycode x) => x.country == str).code))
							{
								break;
							}
							this.label1.Invoke(new MethodInvoker(() => {
				...
				else
				{
					...
					while (true)
					{
						...
						ssh aqua = this.listssh.FirstOrDefault<ssh>((ssh x) => (!(x.live != "dead") || x.used ? false : x.country == text1));
						if (aqua == null)
						{
							MessageBox.Show("All SSH are used or dead, please update new SSH list!", Application.ProductName, MessageBoxButtons.OK, MessageBoxIcon.Hand);
						...
						while (true)
						{
							if ((this.listssh.ElementAt<ssh>(num2).live == "dead" || this.listssh.ElementAt<ssh>(num2).used ? false : this.listssh.ElementAt<ssh>(num2).country == text1))
							{
								break;
							}
						...
						this.curip = aqua.IP;
						if (sshcommand.SetSSH(aqua.IP, aqua.username, aqua.password, this.ipAddressControl1.Text, this.numericUpDown1.Value.ToString(), ref this.bitproc))
						{
							break;
						}
						aqua.live = "dead";
						...
						try
						{
							...
							if (end != "hetssh")
							{
								...
								if (strArrays.Count<string>() >= 4)
								{
									...
									if (!sshcommand.SetSSH(strArrays[1], strArrays[2], strArrays[3], this.ipAddressControl1.Text, this.numericUpDown1.Value.ToString(), ref this.bitproc))
									{
										string str4 = string.Concat(this.sshserverurl, "/Home/xoassh?ID=", strArrays[0]);
										HttpWebRequest httpWebRequest1 = (HttpWebRequest)WebRequest.Create(str4);
										httpWebRequest1.UserAgent = "autoleadios";
										try
										{
		...
		Label3:
            ...
            //goto Label2;
            MessageBox.Show("No Lable2 to goto line 10572!", Application.ProductName, MessageBoxButtons.OK, MessageBoxIcon.Hand);
        }
